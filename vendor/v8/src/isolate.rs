...
// LODESTONE PATCH: since Rust 1.72 `TypeId` is 128 bits and how it feeds a
// `Hasher` is an unstable std detail (`write_u64` once, several times, or raw
// bytes). Fold whatever we receive into the state instead of assuming exactly
// one `write_u64` call.
impl Hasher for TypeIdHasher {
  ...
  fn write_u64(&mut self, value: u64) {
    self.state = Some(match self.state {
      None => value,
      Some(prev) => prev.rotate_left(29) ^ value,
    });
  }
